...
import { logger, Logger, LogLevel, LogEntry, LoggerConfig } from './logger';
...
describe('Logger工具测试', () => {
  ...
  describe('批量日志测试', () => {
    test('应该正确处理批量日志记录', () => {
      const entries: Omit<LogEntry, 'timestamp'>[] = [
        { level: LogLevel.INFO, message: '批量消息1', data: { id: 1 } },
        { level: LogLevel.WARN, message: '批量消息2', data: { id: 2 } },
        { level: LogLevel.ERROR, message: '批量消息3', data: { id: 3 } }
      ];
    ...
    test('应该在批量日志中应用级别过滤', () => {
      ...
      const entries: Omit<LogEntry, 'timestamp'>[] = [
        { level: LogLevel.DEBUG, message: '不应该记录' },
        { level: LogLevel.INFO, message: '也不应该记录' },
        { level: LogLevel.WARN, message: '应该记录' },
        { level: LogLevel.ERROR, message: '也应该记录' }
      ];
    ...
    test('应该为批量日志设置正确的时间戳', () => {
      ...
      const entries: Omit<LogEntry, 'timestamp'>[] = [
        ...
        { level: LogLevel.INFO, message: '消息2' }
      ];
  ...
  describe('内存管理测试', () => {
    test('应该维持最大日志数量限制', () => {
      testLogger = new Logger({ maxLogs: 3 });
  ...
  describe('文件日志集成测试', () => {
    test('应该调用文件日志服务写入单条日志', async () => {
      ...
      expect(mockFileLoggerService.writeLog).toHaveBeenCalledWith(
    ...
    test('应该调用文件日志服务写入批量日志', async () => {
      ...
      const entries: Omit<LogEntry, 'timestamp'>[] = [
        { level: LogLevel.INFO, message: '批量消息1' },
        { level: LogLevel.WARN, message: '批量消息2' }
      ];
      ...
      expect(mockFileLoggerService.writeLogBatch).toHaveBeenCalledWith(
