...
class GlobalErrorHandler {
  ...
  private setupConsoleErrorCapture(): void {
    ...
    console.error = (...args: any[]) => {
      ...
      try {
        ...
        const errorReport: ErrorReport = {
          type: 'console',
          message: `Console Error: ${_message}`,
          url: window.location.href,
          userAgent: navigator.userAgent,
          timestamp: new Date(),
          sessionId: this.sessionId,
        ...
        if (_errorArg) {
          errorReport.stack = _errorArg.stack;
        }
  ...
  private setupResourceErrorHandler(): void {
    window.addEventListener('error', (event) => {
      if (event.target !== window) {
        ...
        const errorReport: ErrorReport = {
          type: 'network',
          message: `Resource failed to load: ${_target.tagName}`,
          source: (_target as any).src || (_target as any).href,
          url: window.location.href,
          userAgent: navigator.userAgent,
          timestamp: new Date(),
          sessionId: this.sessionId,
  ...
  private generateErrorKey(errorReport: ErrorReport): string {
    ...
    return _parts.join('|').replace(/\s+/g, ' ').trim();
  }
  ...
  private logError(errorReport: ErrorReport): void {
    try {
      ...
      switch (errorReport.type) {
        case 'javascript':
        case 'promise':
          logger.error(`${errorReport.type.toUpperCase()} Error: ${errorReport.message}`, _logData);
          break;

        case 'console':
          logger.warn(`Console captured: ${errorReport.message}`, _logData);
          break;

        case 'network':
          logger.warn(`Resource loading failed: ${errorReport.message}`, _logData);
          break;

        default:
          logger.error(`Unknown error type: ${errorReport.message}`, _logData);
      }
    } catch (error) {
